...
- ページ: `/`（トップ）、`/demo/:patternId`（2パネルデモ）、`/slides`（17枚スライド相当）。
- デモパターン: 30パターン定義済み。
- 左パネル: SVG簡略マップ、多角形描画、ズーム、パン、クリック位置選択、距離スケール、凡例。
- 右パネル: ID入力、サンプルIDチップ、商品表示、同一ID複数店舗時の店舗選択。
- 数字ID: 1〜8桁のみ許可。

## 3. データ要件（今回）

- 永続DBは使わず、コード内データ（ハードコード）で実装。
- 店舗ごとの多角形エリアと商品データを保持。
- 30パターンをメタ定義し、店舗・商品・エリアを生成。

## 4. 画面要件

### 4.1 トップページ

- サービス概要、料金要約、機能アピール、導入効果、特許情報、30パターン一覧、スライド導線。

### 4.2 デモページ

- 上部情報バー: パターン情報、切替ドロップダウン、トップ戻り。
- 左: 地図。
- 右: スマホUI。
- 検索時、現在位置とIDで多角形内包判定（Ray Casting）し、結果連動表示。
...
- モダンブラウザ対応（Chrome/Edge/Safari/Firefox 最新）。
- スマホ幅（767px以下）では縦積み表示。
- 外部地図API不使用。

## 6. あいまい点の実装判断（今回）

1. 30パターンは「実在業務データ」ではなく、デモ生成データとして実装。
2. Webスライドは Reveal.js ではなく、要件タイトル準拠のカードUIで実装。
3. オークション上位表示は説明対象に留め、UIロジックとしては未実装。
4. 料金はデモ参照表示（桁数ベース）を優先。

## 7. 今後の拡張ポイント

- 30パターン個別の手作業チューニング（座標・商品文言）。
- フィルターUI（用途/規模/桁数）追加。
- スライドを本番プレゼン向けにテンプレート化。
- Vercel本番運用向け監視・分析導入。
